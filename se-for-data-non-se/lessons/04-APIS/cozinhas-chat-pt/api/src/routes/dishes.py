...
async def upload_dish_image(image: UploadFile = File(...)):
    if not image.content_type or not image.content_type.startswith("image/"):
        raise HTTPException(status_code=400, detail="Arquivo precisa ser uma imagem")

    suffix = Path(image.filename or "").suffix.lower()
    if suffix not in {".jpg", ".jpeg", ".png", ".webp"}:
        raise HTTPException(status_code=400, detail="Formato aceito: jpg, jpeg, png, webp")

    UPLOAD_DIR.mkdir(parents=True, exist_ok=True)
    filename = f"dish-{uuid4().hex}{suffix}"
