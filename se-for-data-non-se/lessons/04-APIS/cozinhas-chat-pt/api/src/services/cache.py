...
def generate_cache_key(prefix: str, **kwargs) -> str:
    ...
    hash_obj = hashlib.md5(str(sorted_params).encode())
...
async def invalidate_hall_cache():
    await cache.delete("list_tables:*")
    await cache.delete("get_table:*")
    await cache.delete("list_hall_events:*")


async def invalidate_kitchen_cache():
    await cache.delete("list_kitchen_tickets:*")
    await cache.delete("get_kitchen_ticket:*")
    await cache.delete("list_kitchen_events:*")
