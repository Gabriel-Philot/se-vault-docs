...
### [2026-02-22T23:22:27+00:00] Architecture canvas visual cleanup + final Biblioteca polish

- Architecture (`api/src/routes/architecture.py`, `frontend/src/pages/Architecture.tsx`)
  - Removed the blurred/duplicated node-image look by splitting node rendering into two layers: circular image thumbnail token + separate label plaque below (no backdrop blur over the image).
  - Simplified architecture section layering to avoid duplicate background image feeling (outer shell no longer reuses the same `architectureBg` hero image).
  - Recalibrated diagram node positions and edge polylines for a cleaner main ingress path (`client -> nginx -> gunicorn -> fastapi`) and better branch balance.
  - Iterated on node visual scale and alignment:
    - increased node circle thumbnails for better use of canvas space,
    - moved `User / Browser` further left and adjusted `client -> nginx` connector line,
    - corrected architecture worker metaphor to use `Erebor` (forge/processing) instead of `Eagles`.
- Biblioteca (`frontend/src/pages/Library.tsx`)
  - Finalized section presentation polish:
    - added divider lines + spacing between section headers and content blocks for `Leaderboard de Heróis` and `Explorador SQL`,
    - darkened top Biblioteca scene while keeping large wrappers transparent and blur only on small text/stat cards,
    - brightened SQL/Postgres backdrop to restore image detail while preserving readability.
- Validation / deploy
  - `python3 -m compileall api/src`
  - `npm run build` (frontend)
  - repeated `docker compose up --build -d api frontend` / `frontend` deploys during visual iterations
